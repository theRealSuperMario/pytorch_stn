...
samples = [
    create_sample(sample_digits(3, X_train, Y_train, ORG_SHP)[0], OUT_SHP,).reshape(-1)
    for i in range(400)
]
...
X_train, y_train = create_dataset(
    N_TRAIN, mnist_data["X_train"], mnist_data["y_train"], ORG_SHP, OUT_SHP
)
X_valid, y_valid = create_dataset(
    N_VALID, mnist_data["X_valid"], mnist_data["y_valid"], ORG_SHP, OUT_SHP
)
X_test, y_test = create_dataset(
    N_TEST, mnist_data["X_test"], mnist_data["y_test"], ORG_SHP, OUT_SHP
)
np.savez_compressed(
    outfile,
    X_train=X_train,
